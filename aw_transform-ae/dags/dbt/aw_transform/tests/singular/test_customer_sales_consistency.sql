...
with
    api_customer_sales as (
        ...
        from {{ ref('stg_api__sales_order_detail') }} as sales_order_detail
        inner join
            {{ ref('stg_api__sales_order_header') }} as sales_order_header
            on sales_order_detail.sales_order_id
    ...
    , db_customer_sales as (
        ...
        from {{ ref('stg_db__sales_order_detail') }} as sales_order_detail
        inner join {{ ref('stg_db__sales_order_header') }} as sales_order_header
            on sales_order_detail.sales_order_id
            = sales_order_header.sales_order_id
        group by customer_id
    )
    ...
    , inconsistent_customers as (
        select
            coalesce(
                api_customer_sales.customer_id, db_customer_sales.customer_id
            ) as customer_id
            ...
            , case
                when
                    api_customer_sales.customer_id is null
                    then 'Missing in API'
