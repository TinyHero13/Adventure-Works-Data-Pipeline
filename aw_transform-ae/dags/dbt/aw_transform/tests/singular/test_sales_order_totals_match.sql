...
with 
    ...
    , order_totals_from_agg as (
        select
            ...
            , order_total_amount
        from {{ ref('fact_sales') }}
        group by sales_order_id, order_total_amount
    )

    , mismatched_totals as (
        select
            order_totals_from_details.sales_order_id
        ...
        join order_totals_from_agg on order_totals_from_details.sales_order_id = order_totals_from_agg.sales_order_id
